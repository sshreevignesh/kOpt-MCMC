...
#Used to calculate the score of a max flow solution_set
def get_val(combined_currcapacity):
    val = 0
    for edge in combined_currcapacity:
        if edge[0] =='virtual_source' or edge[0]=='virtual_sink' or edge[1] =='virtual_source' or edge[1]=='virtual_sink':
            continue
        val += alpha*(abs(combined_currcapacity[edge])*MCMCconfig.EdgeLength[edge])+(1-alpha)*(combined_currcapacity[edge]**4)
    return val

def get_val1(combined_currcapacity):
    val1 = 0
    val2 = 0
    for edge in combined_currcapacity:
        if edge[0] =='virtual_source' or edge[0]=='virtual_sink' or edge[1] =='virtual_source' or edge[1]=='virtual_sink':
            continue
        val1 += abs(combined_currcapacity[edge])*MCMCconfig.EdgeLength[edge]
        val2 += combined_currcapacity[edge]**4
    return val1,val2


def calculate_probability(newpath,oldpath,lamda,combined_currcapacity):
    val1 = get_val(combined_currcapacity)
    for i in oldpath:
        if i not in newpath:
            combined_currcapacity[i]-=1
            combined_currcapacity[(i[1],i[0])]-=1
    for i in newpath:
        if i not in oldpath:
            combined_currcapacity[i]+=1
            combined_currcapacity[(i[1],i[0])]+=1
    val2 = get_val(combined_currcapacity)
    for i in oldpath:
        if i not in newpath:
            combined_currcapacity[i]+=1
            combined_currcapacity[(i[1],i[0])]+=1
...
#        if random.random() <= calculate_probability(newpath, path, lamda,combined_currcapacity=combined_currcapacity):
...
if __name__ == "__main__":
    ...
    filepath = "newkoptmcmcmulti_islington" 
    if isMultiSource:
        filepath = filepath+"virtual"
    
    with open(filepath + 'numsol.csv',mode = 'a') as f:
        fwriter = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        fwriter.writerow([alpha,lamda,num_iter,pathlength,final_kval,get_val1(combined_currcapacity)[1],exectime])
        f.close() 
        
    filepath = "newkoptmcmcmulti_islington" + str(int(alpha*100))+"A" + str(num_iter) + "I" + str(int(lamda*100)) + "l"
    if isMultiSource:
        filepath = "newkoptmcmcmulti_islingtonvirtual" + str(int(alpha*100))+"A" + str(num_iter) + "I" + str(int(lamda*100)) + "l"
    if os.path.exists(filepath) == False:
        os.makedirs(filepath)
    f = open(filepath + "/output"+str(runnumber), "w")
